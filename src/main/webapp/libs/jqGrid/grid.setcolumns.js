            ...
            if ( $("#"+IDs.themodal).html() != null ) {
                ...
                for(i=0;i<this.p.colNames.length;i++){
                    if(!$t.p.colModel[i].hidedlg) { // added from T. Tomov
                        ...
                        formdata += "<tr><td style='white-space: pre;'><input type='checkbox' style='margin-right:5px;' id='col_" + this.p.colModel[i].name+rand + "' class='cbox' value='T' " + 
                        ((this.p.colModel[i].hidden===false)?"checked":"") + "/>" +  "<label for='col_" + this.p.colModel[i].name +rand+ "'>" + this.p.colNames[i] + ((p.colnameview) ? " (" + this.p.colModel[i].name + ")" : "" )+ "</label></td></tr>";
                    }
                }
                ...
                bC  ="<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default ui-corner-all'>关闭</a>";
                ...
                if(!p.updateAfterCheck) {
                    ...
                    $(":input","#"+dtbl).click(function(e){
                        ...
                        if(cn){
                        	cn = cn.substring(0, cn.indexOf("--rand--"));
                            if(this.checked) {
                                $($t).jqGrid("showCol",cn);
                                var data = storage.local("show-columns-"+gID) ||"";
                                if(!data.contains("/"+cn+"/"))
                                	storage.local("show-columns-"+gID,data+"/"+cn+"/");
                            } else {
                                $($t).jqGrid("hideCol",cn);
                                var data = storage.local("show-columns-"+gID)||"";
                                storage.local("show-columns-"+gID,data.replace("/"+cn+"/","/"));
                            }
                            if(p.ShrinkToFit===true) {
                            	//zhuhaiyang add begin
                            	var grid = $($t);
                            	var width = grid.data('orignial-width') || $t.grid.width-0.001;
                            	grid.jqGrid("setGridWidth",width,true);
                                //zhuhaiyang add end
                            	//$($t).jqGrid("setGridWdth",$t.grid.width-0.001,true); zhuhaiyang comments out
                            }
                    		var allColumns = $($t).jqGrid('getGridParam','colModel');
                    		var newWidth = [];
                        	for(var i = 0 ; i < allColumns.length;i++){
                        		newWidth.push(allColumns[i].width||0);
                        	}
                        	storage.local('columns-width-'+$($t).attr('id'),newWidth.join(','));
                        }
                        return this;
                    });
                }
                $("#eData", "#"+dtbl+"_2").click(function(e){
                	console.log("hide"+gID+" "+IDs.themodal);
