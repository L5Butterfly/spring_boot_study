...
function() {
	...
	C.prototype = [], C.prototype.constructor = C, w.extend = function() {
		...
		}), i = l.now < l.min || l.now > l.max || (o.isWorkDay && !r.isWorkDay(c.year, c.month, c.date)), e && e[i ? "addClass" : "removeClass"](s), i
	},
	T.prototype.isWorkDay = function(y,m,d){
    	//周末校验
    	 var tmpDate = new Date();
         tmpDate.setFullYear(y);
         tmpDate.setMonth(m);
         tmpDate.setDate(d);
         var data = {"time":tmpDate};
...
//        	 flag = true;
//         }
         //校验节假日配置 返回值true 为节假日，false，工作日
         $.ajax({
        	 url:'/applyProcess/checHoliday.do',
        	 data:data,
        	 async: false,  
	...
	T.prototype.calendar = function(e) {
		...
		if (l.year < d[0] && (l.year = d[0], r.hint("最低只能支持到公元" + d[0] + "年")), l.year > d[1] && (l.year = d[1], r.hint("最高只能支持到公元" + d[1] + "年")), r.firstDate || (r.firstDate = w.extend({}, l)), c.setFullYear(l.year, l.month, 1), t = c.getDay(), a = n.getEndDate(l.month, l.year), i = n.getEndDate(l.month + 1, l.year), 
...
function getLastTime(obj){
	if(obj.isWorkDay){
		if(count == 0){
			date = checkConfig(obj);
			var arry = date.split("-");
			date = {year:arry[0], month:arry[1]-1, date:arry[2]};
			count ++;
		}
		
	}else{
		date = {year:2099, month:12, date:31};
	}
	return date;
}
function checkConfig(obj){
		var data = {"starttime":obj.min.year+"-"+(obj.min.month+1)+"-"+obj.dateTime.date,"endtime":obj.max.year+"-"+(obj.max.month+1)+"-"+obj.max.date};
		var date ;
		$.ajax({
	 		url:'/applyProcess/checkConfig.do',
	 		data:data,
	 		async: false,  
	 		success:function(data){
	 			date = data;
