...
//						this.toggle_node(e.target);
						/**高冬冬修改 START **/
						if(this.settings.plugins.indexOf('checkbox') === -1)
							this.toggle_node(e.target);
						/**高冬冬修改 END**/
					}, this))
				.on("click.jstree", ".jstree-anchor", $.proxy(function (e) {
		...
		/**
		 * 高冬冬添加,重新绑定jstree的所有事件.
		 * 其它js中绑定在jstree上的事件会被卸载,需要重新绑定
		 * rebind all events
		 * @private
		 * @name _rebind()
		 */
		...
		trigger : function (ev, data) {
			...
			data.instance = this;
			/*****/
			this.element != undefined && this.element.triggerHandler(ev.replace('.jstree','') + '.jstree', data);
			/***
			朱海洋修改，Uncaught TypeError: Cannot read property 'triggerHandler' of null
			当tree还在多次展开时，destory掉，会有这个错
		...
		get_state : function () {
			var state	= {
				'core' : {
					'open' : [],
					'scroll' : {
						'left' : this.element != undefined && this.element.scrollLeft(),
						'top' : this.element != undefined && this.element.scrollTop()
						/***
						朱海洋修改，Uncaught TypeError: Cannot read property 'triggerHandler' of null
						当tree还在多次展开时，destory掉，会有这个错
						this.element.triggerHandler(ev.replace('.jstree','') + '.jstree', data);
...
//			this._data.checkbox.selected = [];
			/**高冬冬 修改START**/
			this._data.checkbox.uto = this._data.checkbox.uto || false;
			this._data.checkbox.selected = this._data.checkbox.selected || [];
			/**高冬冬 修改End**/
		 ...
		 * 高冬冬添加
		 * 其它js中绑定在jstree上的事件会被卸载,需要重新绑定
		 * enable cascade ,If three_state is set to true this setting is automatically set to 'up+down+undetermined'.
		 * @name enable_three_state()
		 * @plugin checkbox
		 */
		this.enable_three_state = function(callback){
			this.settings.checkbox.three_state = true;
			this._rebind();
			if(callback) callback.call(this);
		}
		 ...
		 * 其它js中绑定在jstree上的事件会被卸载,需要重新绑定
		 * disable cascade
		 * @name disable_three_state()
		 * @plugin checkbox
		 */
		this.disable_three_state = function(callback){
			this.settings.checkbox.three_state = false;
